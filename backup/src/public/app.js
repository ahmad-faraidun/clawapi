...
async function executeProviderAction(btn, action, providerName) {
    ...
    try {
        const res = await fetch(`/api/providers/${action}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ provider: providerName, port: els.serverPort.value || 8855 })
        });
        const data = await res.json();
        ...
        if (data && data.success) {
            if (action === 'test') {
                ...
                if (action === 'auth') {
                    showModal({ 
                        ...
                        message: 'An authenticator window has been launched.\nPlease complete the login in the new window while keeping this dashboard open.', 
                        type: 'info' 
                    });
                }
            }
            ...
            if (action === 'test') {
                showModal({ title: `Test Failed: ${providerName}`, message: data.error || 'Unknown error occurred.', type: 'error' });
            }
        }
    } catch (err) {
        showToast(err.message, 'error');
